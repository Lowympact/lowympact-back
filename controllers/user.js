		...
		if (userId == req.params.userId) {
			...
			if (typeStatistic == "ecoscore") {
				...
				switch (typeAggregate) {
					case "fromBeginning":
						dateIntervalStep = Math.floor(
							(now - user.createdAt) / (1000 * 3600 * 24)
						);
					...
					default:
						res.status(400).json({
							success: false,
							message:
								"Unknown or undefined type of agregate (typeAggregate)",
						});
						next();
						break;
				}
				...
				user.cart.forEach((el) => {
					...
					if (
						index < statistics.carbonImpact.data.length &&
						el.carbonImpact >= 0
					) {
						statistics.carbonImpact.data[index].nbProducts +=
							el.quantity;
						statistics.carbonImpact.data[index].impact +=
							el.quantity * el.carbonImpact;
					} else if (index < statistics.carbonImpact.data.length) {
						statistics.carbonImpact.data[index].nbProductUnknow +=
							el.quantity;
					}
				});
			} else {
				res.status(400).json({
					success: false,
					message:
						"Unknown or undefined type of statistics (typeStatistic)",
				});
				next();
			}
			...
			if (req.body.bcProductAddress) {
				previousMatchingProduct = user.history.find(
					(t) =>
						...
						t.bcProductAddress == data.bcProductAddress
				);
			} else {
				previousMatchingProduct = user.history.find(
...
exports.updateCart = async (req, res, next) => {
	try {
		if (req.jwt.id == req.params.userId) {
			...
			if (
				req.body.quantityDelta == undefined ||
				typeof req.body.quantityDelta == "string"
			) {
				res.status(400).json({
			...
			if (
				req.body.carbonImpact == undefined ||
				typeof req.body.carbonImpact == "string"
			) {
				res.status(400).json({
			...
			if (!item) {
				...
				if (req.body.quantityDelta >= 1) {
					...
					res.status(400).json({
						message:
							"You can't set a negative quantity for a new item",
					});
					next();
				}
			} else {
				if (req.body.quantityDelta + item.quantity < 0) {
					res.status(400).json({
						message:
							"You can't remove more product than there are in the cart",
					});
					next();
				} else {
